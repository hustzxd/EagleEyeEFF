...
def main_worker(gpu, ngpus_per_node, args):
    ...
    flops_ori, params_ori, df_ori = distiller.performance_summary(model, dummy_input=torch.rand(1, 3, 32, 32))  # TODO
    logger.info("\n{}\n".format(df_ori))
    ...
    if args.hp.rcp.HasField("freeze_sparsity"):
        ...
        pruned_flops, pruned_params, pruned_df = distiller.performance_summary(
            pruned_model, dummy_input=torch.rand(1, 3, 32, 32)
        )
    ...
    if args.hp.rcp.HasField("ada"):  # differential_evolution
        ...
        val_fun = partial(validate, val_loader=val_loader, criterion=criterion, args=args)
    ...
    for epoch in range(0, args.hp.epochs):
        ...
        # evaluate on validation set
        acc1, acc5 = validate(model=pruned_model, val_loader=val_loader, criterion=criterion, args=args)
